...
1.class和struct类的区别
2.空类
3.this指针，每一个非静态成员函数的参数 
4.构造函数
    1.不写编译会默认生成一个默认构造函数，只会调用自定义类型的构造函数初始化自定义类型
    2.默认构造函数就是不用传参数的构造函数，全缺省，无参数，编译器生成的
    3.c++必须有一个构造函数
    4.
    5.c++构造函数一般是全缺省

5.析构函数
    1.不写编译会默认生成一个默认析构函数，只会调用自定义类型的析构函数清理自定义类型
    2.默认构造函数无参数
    3.c++析构函数不一定有的（因为不一定有资源要清理）
 */
...
// this指针可以为空吗？
/*
 *在 C++ 面向对象语义中，this 指针是每个非静态成员函数隐式拥有的一个指针参数，它指向“当前调用该成员函数的对象实例”。
 */
